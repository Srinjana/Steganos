...
Original file is located at
    https://colab.research.google.com/drive/1ECPMJMlJ0iZnpubcWpaEHdeF_0aOEgKG
"""
...
import numpy as np
import types
from google.colab.patches import cv2_imshow #Google colab crashes if you try to display image using cv2.imshow() thus use this import
...
def hideData(image, secret_message):
  ...
  if (len(secret_message) > n_bytes):
    raise ValueError(" ERROR, need a bigger image or smaller data")
  
  secret_message += "#####" # Using this string as delimiter
  data_index = 0
...
def encode_text(): 
  image_name = input("Enter the image filename (with extension): ") 
  ...
  print("The dimensions of the image is: ",image.shape) #check the shape of image to calculate the number of bytes in it
  ...
  data = input("Enter secret message to be encoded : ") 
  if (len(data) == 0): 
    raise ValueError('No Data present')
  
  filename = input("Enter the name of new encoded image file (with extension): ")
...
def decode_text():
  ...
  image_name = input("Enter the filename of the image that you want to decode (with extension) :") 
...
def Steganography(): 
    ...
    if (userinput == 1):
      ...
      print("\nDecoding....") 
      print("The Decoded message is " + decode_text()) 
    else: 
